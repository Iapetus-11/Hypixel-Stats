...
class Useful(commands.Cog):
    ...
    async def information(self, ctx):
        info_msg = discord.Embed(color=await self.bot.cc(ctx.author.id))
    ...
    async def stats(self, ctx):
        ...
        embed.set_author(name="Bot Statistics",
                         icon_url=str(self.bot.user.avatar_url_as(format="png", size=256)))
        try:
            key_data = (await asyncio.wait_for(await self.cache.hypixel.getKeyData(), timeout=2))['record']
        except Exception:
    ...
    async def useful_links(self, ctx):
        ...
        embed = discord.Embed(description=desc, color=await self.bot.cc(ctx.author.id),
                              title=":link: Useful Links :link:")
    ...
    async def do_math(self, ctx):
        try:
            ...
            await ctx.send(
                embed=discord.Embed(color=await self.bot.cc(ctx.author.id), description="Oops, something went wrong."))
