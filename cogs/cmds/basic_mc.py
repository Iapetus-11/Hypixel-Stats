...
class BasicMC(commands.Cog):
    ...
    async def skinner(self, ctx, *, gamertag: str):
        ...
        if response.status == 204:
            await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description="That player doesn't exist!"))
            return
        ...
        if "error" in content:
            if content["error"] == "TooManyRequestsException":
                await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description="Hey! Slow down!"))
                return
        if len(content["properties"]) == 0:
            await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description="This user's skin can't be stolen for some reason..."))
            return
        undec = base64.b64decode(content["properties"][0]["value"])
        try:
            url = json.loads(undec)["textures"]["SKIN"]["url"]
        except Exception:
            await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description="An error occurred while fetching that skin!"))
            return
        skin_embed = discord.Embed(color=self.bot.CONFIG["cc"], description=f"{gamertag}'s skin\n[**[Download]**]({url})")
    ...
    async def get_uuid(self, ctx, *, gamertag: str):
        ...
        if not j:
            await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description="That user could not be found."))
            return
        await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description=f"{gamertag}: ``{j[0]['id']}``"))
    ...
    async def get_gamertag(self, ctx, *, uuid: str):
        ...
        name = j[len(j)-1]["name"]
        await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description=f"{uuid}: ``{name}``"))
    ...
    async def mc_sales(self, ctx):
        ...
        await ctx.send(embed=discord.Embed(color=self.bot.CONFIG["cc"], description=f"**{j['total']}** total Minecraft copies sold, **{round(j['saleVelocityPerSeconds'], 3)}** copies sold per second."))

    @commands.command(name="colorcodes", aliases=["mccolorcodes", "colors", "cc"])
    async def mc_color_codes(self, ctx):
        embed = discord.Embed(color=self.bot.CONFIG["cc"], description="Text in Minecraft can be formatted using different codes and\nthe section (``§``) sign.")
